...
- "Point Blank" → ONLY for bow or projectile attack builds. Never take on melee or spell builds.
- "Avatar of Fire" → converts ALL damage to fire; build must deal 100% fire damage. Do NOT use with physical or other elemental damage scaling.
...

{
  ...
  "passive_notables": [
    "20-30 notable names from the MAIN passive tree only. NO MORE THAN 1 KEYSTONE TOTAL.",
    "Keystones have HARD prerequisites — if the build does not fully commit, omit the keystone entirely: Chaos Inoculation = pure ES build with zero life investment; Resolute Technique = build that cannot reliably crit AND has no crit nodes whatsoever; Elemental Overload = elemental build that intentionally avoids ALL crit investment; Ghost Reaver = ES build with explicit ES leech; Vaal Pact = build with reliable leech wanting instant leech; Mind Over Matter = large dedicated mana pool as a damage buffer; Acrobatics = evasion build accepting loss of block chance; Point Blank = bow/projectile attack build only; Avatar of Fire = 100% fire damage build only.",
